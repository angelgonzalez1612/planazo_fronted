...
export function PlanCard({ plan, icon, categoryLabel }: { plan: Plan; icon: string; categoryLabel: string }) {
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card">
      <div className="relative aspect-4/3 bg-secondary">
        <Image src={cover.url} alt={cover.alt} fill sizes="(min-width:1024px) 25vw, (min-width:640px) 50vw, 100vw" className="object-cover" />
        <div className="absolute inset-0 flex items-start justify-between p-3">
          ...
          <button
            type="button"
            title="Guardar"
            onClick={() => triggerAuthPrompt("guardar", () => toggleFavorite(plan.id))}
      ...
      <div className="flex flex-1 flex-col gap-2 p-4.5 pb-5">
        ...
        <p className="flex items-center gap-1.5 text-sm text-ink-soft">📍 {plan.zone}</p>
        {plan.kind === "evento" && (
          <p className="flex items-center gap-1.5 text-[13px] font-semibold text-brand-deep">📅 {plan.dateLabel}</p>
        )}
