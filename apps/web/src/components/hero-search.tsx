  ...
  return (
    <div>
      <div ref={boxRef} className="relative">
        <form
          onSubmit={(e) => {
            ...
            else document.getElementById("destacados")?.scrollIntoView({ behavior: "smooth" });
          }}
        ...
        >
          ...
          />
          <button type="submit" className="rounded-[14px] bg-ink px-6 py-3.5 text-[15px] font-bold text-white">
            Buscar
          </button>
        </form>
        ...
        {open && (
          <div className="absolute top-[calc(100%+8px)] right-0 left-0 z-30 max-h-[360px] overflow-y-auto rounded-2xl border border-border bg-card p-2 shadow-[0_24px_50px_-24px_rgba(25,21,18,0.35)]">
            {matches.length === 0 ? (
              <div className="p-4 text-center text-sm text-ink-soft">Sin resultados para &ldquo;{query}&rdquo;</div>
            ) : (
              matches.map((s, i) => (
                <button
                  key={i}
                ...
                >
                  ...
                  <span className="min-w-0">
                    <span className="block text-[14.5px] font-semibold">{s.label}</span>
                    <span className="mt-px block text-[12.5px] text-ink-soft">{s.sub}</span>
                  </span>
      ...
      <div className="mt-3.5 flex flex-wrap items-center gap-2">
        <span className="p-0.5 text-[13px] font-medium text-ink-soft">Prueba:</span>
        {[0, 1, 2, 3].map((slot) => (
          <button
            key={slot}
            type="button"
            onClick={() => setQuery(examples[(exampleSlot + slot) % examples.length])}
