...
export function SiteHeader() {
  ...
  useEffect(() => {
    function onClickOutside(e: MouseEvent) {
      if (
        cityMenuRef.current &&
        !cityMenuRef.current.contains(e.target as Node)
      ) {
  ...
  return (
    <header
      className={`sticky top-0 z-40 border-b border-border font-sans transition-[box-shadow,background-color] duration-200 ${
        isScrolled
          ? "bg-background/97 shadow-[0_10px_30px_-20px_rgba(25,21,18,0.35)]"
          : "bg-background/88"
      } backdrop-blur-md`}
    >
      <div
      ...
      >
        <Link
          href="/"
          className="group flex items-center gap-0.5 font-heading text-2xl font-extrabold tracking-tight"
        >
          <Image
            src="/logo.png"
            alt=""
        ...
        <div className="hidden items-center gap-3 md:ml-auto md:flex">
          ...
          <div className="relative" ref={cityMenuRef}>
            ...
            >
              ...
              {city}
              <span
                className={`text-[9px] text-ink-soft transition-transform ${cityMenuOpen ? "rotate-180" : ""}`}
              >
                ▾
              </span>
            </button>
